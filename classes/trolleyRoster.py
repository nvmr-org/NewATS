...
class TrolleyRoster(object):
    ...
    def dumpXml(self):
        if TrolleyRoster.__eTrace : logger.info("Enter trolleyRoster.dumpXml")
        trolleyXml = ET.Element('trolleyRoster')
        trolleyXml.set('version', '1.0')
        trolleyXml.append(ET.Comment('Generated by NVMR Automated Trolley Sequencer'))
        trolleyXml.append(ET.Comment('Create a trolley roster that consists of consecutive trolleys on a layout'))
        title = ET.SubElement(trolleyXml, 'title')
        title.text = self.title
        dateCreated = ET.SubElement(trolleyXml, 'dateCreated')
        dateCreated.text = str(datetime.datetime.now())
        dateModified = ET.SubElement(trolleyXml, 'dateModified')
        dateModified.text = str(datetime.datetime.now())
        rosterCount = ET.SubElement(trolleyXml,'rosterCount')
        rosterCount.text = str(self.size())
        roster = ET.SubElement(trolleyXml, 'roster')
        for trly in self._list:
            trolley = ET.SubElement(roster, 'trolley')
            address = ET.SubElement(trolley,'address')
            address.text = str(trly.address)
            maxSpeed = ET.SubElement(trolley,'maxSpeed')
            maxSpeed.text = str(trly.maxSpeed)
            soundEnabled = ET.SubElement(roster,'soundEnabled')
            soundEnabled.text = str(trly.soundEnabled)
            currentPosition = ET.SubElement(trolley,'currentPosition')
            currentPosition.text = str(trly.currentPosition.address)
            currentPositionDescription = ET.SubElement(trolley,'currentPositionDescription')
            currentPositionDescription.text = str(trly.currentPosition.description)
